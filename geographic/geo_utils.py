...
def distance_point_to_polygon(point: Point, poly: Union[Polygon, MultiPolygon]) -> float:
    """
...
def distances_to_point(point: Point, geos: List[BaseGeometry]) -> np.array:
    ...
    Args:
        ...
        geos: List of all Geographic object in the area in degrees
    Returns:
...
def sample_points_in_poly(poly: Polygon, num_samples=1, seed=None) -> List[Point]:
    ...
    # Pretty slow for large (1000+) samples, can parallelize with parmap
...
def geom2image_projection(image: np.array, bbox: tuple, geom: shapely.geometry, color: Union[float, List],
    ...
    """
    import cv2  # noqa - not always used

    transform = lambda coord: [int(image.shape[1] * (coord[0] - bbox[0]) / (bbox[2] - bbox[0])),  # noqa
...
def read_shapefile_to_df(shapefile_path: str) -> pd.DataFrame:
