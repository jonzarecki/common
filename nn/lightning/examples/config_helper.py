...
from torch.optim.lr_scheduler import MultiStepLR, CyclicLR, CosineAnnealingLR, OneCycleLR
...
from common.nn.models.image.resnet import ResNet101, ResNet50
...
def add_models_to_config(args):
    if args.model_name == 'mlp':
        ...
        from common.nn.models import conv_block, Flatten
        args.model_lambda = \
...
def add_optimizers_to_config(args):
    if args.optimizer_name == 'sgd':
        args.optimizer_lambda = lambda model: optim.SGD(model.parameters(), lr=args.lr, momentum=0.9,
                                                        weight_decay=1e-2, nesterov=True)
    elif args.optimizer_name == 'adam':
        args.optimizer_lambda = lambda model: optim.Adam(model.parameters(), lr=args.lr,
                                                         betas=(0.9, 0.99))
    ...
    if args.lr_sched_type == 'constant':
        ...
        args.lr_scheduler_lambda = lambda opt, **kwargs: MultiStepLR(opt, milestones=args.lr_sched_milestones,
                                                                     gamma=args.lr_sched_gamma)
    elif args.lr_sched_type == 'cyclic':
        args.lr_scheduler_lambda = lambda opt, **kwargs: CyclicLR(opt, step_size_up=int(args.epochs / 2),
                                                                  base_lr=1e-4, max_lr=5e-4)
    elif args.lr_sched_type == 'one-cycle':
        args.lr_scheduler_lambda = lambda opt, steps_per_epoch, **kwargs: \
            OneCycleLR(opt, max_lr=args.lr, steps_per_epoch=steps_per_epoch, epochs=args.epochs)
    elif args.lr_sched_type == 'cosine':
        args.lr_scheduler_lambda = lambda opt, **kwargs: CosineAnnealingLR(opt, T_max=args.epochs, eta_min=0.)
