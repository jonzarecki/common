...
def define_argparser():
    ...
    parser.add_argument('--dataset-name', type=str,
                        choices=['mnist', 'fashion-mnist', 'cifar10', 'cifar100', 'mini-imagenet', 'omniglot',
                                 'kmnist', '3class-mnist', 'stanford-cars', 'trec'],
                        default='cifar10', help='name of dataset to train on (default: cifar10)')
    parser.add_argument('--train-noise', type=float, default=0.,
                        help='amount of label noise to add to the training set')
    parser.add_argument('--model-name', type=str,
                        choices=['simple-cnn', 'mlp', 'resnet18', 'vgg11', 'vgg8b', 'bert-gru', 'resnet101', 'resnet50',
                                 'densenet-bc-169', 'wresnet28_10', 'wresnet40_2'],
                        default='resnet18',
    ...
    parser.add_argument('--loss', type=str, choices=['ce', 'partial-ce', 'ce-ls', 'bge-logit', 'scaled'],
                        default='ce', help='loss function')
    ...
    parser.add_argument('--optimizer-name', type=str,
                        choices=['adam', 'sgd', 'adamw', 'adagrad', 'adadelta', 'rmsprop'], default='sgd')
    # lr scheduling
    parser.add_argument('--lr', type=float, default=1e-3, metavar='LR', help='learning rate (default: 1e-3)')
    parser.add_argument('--lr-sched-type', type=str, choices=['multi-step', 'constant', 'cyclic', 'cosine', 'one-cycle']
                        , default='constant', help='lr scheduler type')
    ...
    parser.add_argument('--lr-sched-milestones', nargs='+', type=int, required=False,
                        help='milestones for multi-step lr-sched')
    ...
    parser.add_argument('--gpus', nargs='+', type=int, required=False, default=None,
                        help='gpu ids to use for training')
    parser.add_argument('--train-size', type=int, default=None, metavar='S',
                        help='amount of examples in train (default: no restriction)')
    parser.add_argument('--test-size', type=int, default=None, metavar='S',
                        help='amount of examples in test (default: no restriction)')
    parser.add_argument('--save-model', action='store_true', default=False,
                        help='For Saving the current Model')
    parser.add_argument('--log-eval-freq', type=int, default=2, help='Epoch freq for evaluatiing')
    parser.add_argument('--offline', action='store_true', default=False,
                        help='Runs the model offline using nohup')

    parser.add_argument('--expr-name', type=str, default='',
...
def main():
    ...
    trainer = Trainer(gpus=args.gpus if args.use_gpu else None,  # , distributed_backend='dp',
                      check_val_every_n_epoch=args.log_eval_freq, logger=logger if not using_debugger else False,
                      benchmark=True,
                      max_epochs=args.epochs, min_epochs=args.epochs,
                      num_sanity_val_steps=2,
                      progress_bar_refresh_rate=2,
                      precision=32)  # 16 if args.use_gpu else 32)  # no speed boost ATM, only mem boost
    trainer.fit(model)
