...
class PLModel(pl.LightningModule):
    ...
    def default_weight_initialization(self):
        self.params_without_bn = [params for name, params in self.named_parameters() if
                                  not ('_bn' in name or '.bn' in name)]
    ...
    def log_confusion_matrix(self, test_labels, test_pred):
        if self.logger is not None:
            ...
            figure = plot_confusion_matrix(cm,
                                           class_names=[str(i) for i in range(len(np.unique(test_pred + test_labels)))])
            cm_image = figure_to_image(figure)

            # Log the confusion matrix as an image summary.
            self.logger.experiment.add_image("Confusion Matrix", torch.tensor(cm_image),
    ...
    def _get_dataset(self, train: bool, download: bool):
        ds = self.all_hparams.dataset(root=DATA_LONG_TERM_DIR, train=train, download=download,
                                      transform=self.all_hparams.transforms['train'])
